...
words_df.createOrReplaceTempView("wordTable")
...
startSQL = time.time()
sqlWordCount = spark.sql("SELECT COUNT(*) AS total_words FROM wordTable")  # SQL count
...
#Run 100 times and report count of API wins
...
for i in range(1, 101):  
    ...
    if pysparkPairTime < sqlPairTime:
        print(f"YES!!!! API wins on run {i}!")
        print(f"API: {pysparkPairTime} sec | SQL: {sqlPairTime} sec")
        api_win_count += 1  # Count the win

# Final summary after all runs
print(f"\n API was faster than SQL in {api_win_count} out of 100 runs.")
